...
\begin{teo}[Teorema da Aproximação de Weierstrass]
...
Também perceba que a expressão para \( B_{ n }(x, g) \) é linear na função \( g \).
...
Multiplicando ambos lados por \( p/n \), ficamos com
    \begin{equation}
        ...
        \label{eq: id_1}
    .\end{equation}
Essa é a primeira identidade que usaremos na demonstração, válida para todos \( p, q \in \R \).
...
Com essas ferramentas, podemos então apresentar a
 \begin{proof}[Demonstração do Teorema \ref{thm: weierstrass}]
    ...
    Vamos, então, aproximar \( f \) pelos seus polinômios de Bernstein.
    ...
    .\end{align}
    Aqui fizemos uso das propriedades de \( B_{ n } (x, f) \) discutidas anteriormente.
