...
\begin{tikzpicture}[x=1.5cm, y=1.5cm, >=stealth, scale=0.8, every node/.style={transform shape}]
...
\node [above] at (hidden-1.north) {\( \sigma (\bfw_{ 1 }^{ \transpose }\bfx + \theta_{ 1 }) \)};
\node [below] at (hidden-2.south) {\( \sigma (\bfw_{ N }^{ \transpose }\bfx + \theta_{ N }) \)};

\foreach \l [count=\i] in {1}
  \draw [->] (output-\i) -- ++(3,0)
    node [above, midway] {\( \sum_{ j=1 }^{ N } \alpha_{ j } \sigma (\bfw_{ j }^{ \transpose } \bfx + \theta_{ j }) \)}
    node [right] {\( G ( \bfx ) \)};
